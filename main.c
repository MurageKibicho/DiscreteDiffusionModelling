...
void ForwardPass(int contextLength, int *input, float currentSigmaBar, cJSON *tensorData, unsigned char  *weightData)
{
	...
	for(int headIndex = 0; headIndex < attentionHeadCount; headIndex++)
	{
		...
		memcpy(positionTokenEmbeddingOut_Skip, positionTokenEmbeddingOut_Skip, contextLength * neuralEmbeddingDimension * sizeof(float));
		...
		free(positionTokenEmbeddingOut_Skip);free(LN1_out);free(adaLN_out);
		break;
	}
...
void Test()
{
	...
	for(int timeStep = 0; timeStep < steps+1; timeStep++)
	{
		...
		ForwardPass(contextLength, input, currentSigmaBar, tensorData, weightData);
		break;
	}
