...
{
	cblas_sgemm(CblasRowMajor,CblasNoTrans,CblasTrans,inputRows,outputCols,inputCols,1.0f,input,inputCols,weight,inputCols,0.0f,output,outputCols);

	if(hasBias)
	{
		for(int b = 0; b < inputRows; b++)
		{
			cblas_saxpy(outputCols, 1.0f, bias, 1, output + b * outputCols, 1);
		}
	}
...
void KibichoTensor_ElementWiseMul(int rows, int cols, float *out, float *a, float *b)
{
	for(int i = 0; i < rows; i++)
	{
		for(int j = 0; j < cols; j++)
...
{
for(int i = 0; i < length; i++){array[i] = SiLU(array[i]);}
}
void ApplyGeLU(int length, float *array)
{
for(int i = 0; i < length; i++){array[i] = GeLU(array[i]);}
}
void ApplyExpf(int length, float *array)
{
for(int i = 0; i < length; i++){array[i] = expf(array[i]);}
}
void Softmax(int length, float *scores)
{
	float max = scores[0];for(int i = 1; i < length; i++){if(scores[i] > max){max = scores[i];}}
	float sum = 0.0f;
	for(int i = 0; i < length; i++){scores[i] = expf(scores[i] - max);sum += scores[i];}
...
void StaggeredScore(int rows, int cols, float deltaSigma, float *array)
{
	float expFactor = expf(-deltaSigma);
	for(int row = 0; row < rows; row++)
	{
		float sumScore = 0.0f;
		for(int v = 0; v < cols; v++){sumScore += array[row * cols + v];}
		float correction =((expFactor - 1.0f) /(cols * expFactor))* sumScore;
		for(int v = 0; v < cols; v++){array[row * cols + v] = correction + array[row * cols + v] / expFactor;}
	}
}

void Transition(int rows, int cols, float *output, int *input, float deltaSigma)
{
	float base = (1.0f - expf(-deltaSigma)) / cols;
	float diag = expf(-deltaSigma) + base;
	for(int r = 0; r < rows; r++)
    	{
    		int token = input[r];
    		for(int v = 0; v < cols; v++)
    		{
    			output[r * cols + v] = base;
    		}
    		output[r * cols + token] = diag;
    	}
}

void SampleCategorical(int rows, int cols, int *sample, float *probabilities)
{
	float eps = 1e-10f;
	for(int r = 0; r < rows; r++)
	{
		...
		{
			float p = probabilities[r * cols + c];
			float random = (float)rand() / (float)RAND_MAX;
			float score = logf(p + eps) + -logf(-logf(random + eps) + eps);
			if(score > bestValue)
            		{
            			bestValue = score;
            			bestIndex = c;
...
void ApplyAddScale_InPlace(int rows, int cols, float *x, float *scale, float *residual)
{
	//Scale[1*cols], x and residual same dimension[rows,cols]
	for(int r = 0; r < rows; r++)
	{
		for(int c = 0; c < cols; c++)
		{
			int idx = r * cols + c;  
			x[idx]  = (x[idx] * scale[c]) + residual[idx];
		}
	}
}
...
{
	assert(batchSize == 1);
	int T = sequenceLength;int C = neuralEmbeddingDimension;int H = attentionHeadCount;int D = C / H;
	float *qkv = calloc(T * 3 * C, sizeof(float));
	KibichoTensor_LinearLayer(T, C, 3 * C, qkv, input, qkvWeights, NULL, false);
	float *out = calloc(T * C, sizeof(float));
	float scale = 1.0f / sqrtf((float)D);
	float *scores = calloc(T * T, sizeof(float));
	for(int h = 0; h < H; h++)
	{
		float *Q = qkv + 0 * C + h * D;
		float *K = qkv + 1 * C + h * D;
		float *V = qkv + 2 * C + h * D;
		cblas_sgemm(CblasRowMajor,CblasNoTrans,CblasTrans,T, T, D,scale,Q, 3 * C,K, 3 * C,0.0f,scores,T);
       	//softmax row-wise
       	for(int t = 0; t < T; t++){Softmax(T, scores + t * T);}
		//attention × V = out head (T x D)
		cblas_sgemm(CblasRowMajor,CblasNoTrans,CblasNoTrans,T, D, T,1.0f,scores,T,V,3 * C,0.0f,out + h * D,C);
	}
	KibichoTensor_LinearLayer(T,C,C,projectedOutput,out,projectionWeights,NULL,false);
	free(scores);free(qkv);free(out);
	if(trainPhase){assert(false);}   
}
